...
router.get("/", async (req, res, next) => {
  try {
    ...
    let productApiFeatures = new ProductApiFeatures(Product.find(), req.query)
      .search()
      .filter()
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      //cloudinary set up
      let avaterClous = await cloudinary.v2.uploader.upload(image);
...
router.post("/review/create", authentication, async (req, res, next) => {
  try {
    ...
    const isReviewed = findProduct.reviews.find(
    ...
    let totalRating = findProduct.reviews.reduce(
      (prev, cur) => cur.rating + prev,
      0
    );
