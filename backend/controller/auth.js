...
const registerController = async (req, res, next) => {
  try {
    ...
    if (avater) {
      res
        .status(200)
        .json({ message: "Avater upload Successfully", user: avater });
    }

    if (avater == "") {
      res.status(200).json({ message: "plz upload file ", user: avater });
    }

    const user = await registerService({ name, email, password, avater });
...
const loginController = async (req, res, next) => {
  try {
    ...
    // const option = {
    //   expires: new Date(Date.new() + 1 * 24 * 60 * 60 * 1000),
    //   httpOnly: true,
    // };
...
const forgetPasswordController = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Please Check your email to get reset password link" });
  } catch (err) {
    next(err);
  }
};
